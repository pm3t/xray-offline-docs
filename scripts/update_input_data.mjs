...
const mawbTdNew = `<td className="px-4 py-3 truncate" style={{ maxWidth: columnWidths.mawb }}>
                                                ...
                                                <div className="flex items-center space-x-1 mt-1.5">
                                                    <button onClick={() => handlePrint(cargo)} className="text-gray-500 hover:text-gray-800 transition-all" title="Print">
                                                        <Printer size={14} />
                                                    </button>
                                                    <button
                                                        onClick={() => handleSendToCeisa(cargo.id)}
                                                        disabled={cargo.isSentToCeisa || cargo.totalPcs < getCalculatedPcs(cargo.mawb, cargo.hawb).actual}
                                                        className={\`transition-all \${cargo.isSentToCeisa ? 'text-green-500' : cargo.totalPcs < getCalculatedPcs(cargo.mawb, cargo.hawb).actual ? 'text-gray-300 cursor-not-allowed' : 'text-purple-600 hover:text-purple-800'}\`}
                                                        title={cargo.isSentToCeisa ? "Sent" : "CEISA"}
                                                    >
                                                        {cargo.isSentToCeisa ? <Check size={14} /> : <Send size={14} />}
                                                    </button>
                                                    <button onClick={() => handleEdit(cargo)} className="text-blue-500 hover:text-blue-800 transition-all" title="Edit">
                                                        <Edit size={14} />
                                                    </button>
                                                    <button onClick={() => handleDelete(cargo.id)} className="text-red-500 hover:text-red-800 transition-all" title="Delete">
                                                        <Trash2 size={14} />
                                                    </button>
                                                </div>
                                            </td>`;
if (content.includes(mawbTdOld)) {
    content = content.replace(mawbTdOld, mawbTdNew);
} else {
    // If not exact match, fall back to regex
    content = content.replace(
...
// Note: handle Select All check condition needs to use paginatedList logic since we modified the display array name.
content = content.replace(
