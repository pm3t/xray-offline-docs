...
interface CargoData {
    ...
    isSentToCeisa?: boolean; // New field
...
const InputDataScreen = () => {
    ...
    useEffect(() => {
        const loadData = async () => {
            const savedData = localStorage.getItem('cargoData');
            if (savedData) {
                const list = JSON.parse(savedData);
                setCargoList(list);
            }
            try {
                const history = await scanDB.getAll();
    ...
    const getCalculatedPcs = (mawb: string, hawb: string) => {
        const relatedScans = scanHistory.filter(h => h.mawb === mawb && (h.hawb || '') === (hawb || ''));
        const actual = relatedScans.reduce((sum, s) => sum + (Number(s.qty) || 0), 0);
        const released = relatedScans.filter(s => s.status === 'Release').reduce((sum, s) => sum + (Number(s.qty) || 0), 0);
        const rejected = relatedScans.filter(s => s.status === 'Reject').reduce((sum, s) => sum + (Number(s.qty) || 0), 0);
        return { actual, released, rejected };
    };

    const handleSendToCeisa = (id: string) => {
        const updatedList = cargoList.map(c =>
            c.id === id ? { ...c, isSentToCeisa: true } : c
        );
    ...
    const saveToLocalStorage = (data: CargoData[]) => {
        setCargoList(data);
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
        ...
        setFormData({
            ...formData,
            [name]: name === 'totalPcs' || name === 'totalWeight' || name === 'actualPcs' ? Number(value) : value
        });
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        let updatedList;
        if (editingCargo) {
            updatedList = cargoList.map(c => c.id === editingCargo.id ? newCargo : c);
            toast.success('Data diperbarui');
        } else {
            updatedList = [...cargoList, newCargo];
    ...
    const handleDelete = (id: string) => {
        if (window.confirm('Hapus data ini?')) {
            const updatedList = cargoList.filter(c => c.id !== id);
            saveToLocalStorage(updatedList);
    ...
    const handleBulkDelete = () => {
        ...
        if (window.confirm(`Hapus ${selectedIds.length} data terpilih?`)) {
            const updatedList = cargoList.filter(c => !selectedIds.includes(c.id));
            saveToLocalStorage(updatedList);
            setSelectedIds([]);
    ...
    const handleImportXLS = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = (evt) => {
            ...
            const importedCargo: CargoData[] = data.map((row, index) => ({
                ...
                mawb: row['MAWB'] || '',
                hawb: row['HAWB'] || '',
                totalPcs: Number(row['Total Pcs'] || 0),
                totalWeight: Number(row['Total Weight'] || 0),
                goodsDescription: row['Goods Description'] || '',
                actualPcs: Number(row['Actual Pcs'] || 0),
                remarks: row['Remarks'] || '',
            ...
            const updatedList = [...cargoList, ...importedCargo];
            saveToLocalStorage(updatedList);
    ...
    const handleSyncAPI = async () => {
        ...
        const updatedList = [...cargoList, ...mockApiData];
        saveToLocalStorage(updatedList);
        toast.success('Sinkronisasi API Berhasil', { id: 'sync' });
    };

    const filteredList = cargoList.filter(c =>
        c.mawb.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            <div className="p-6 max-w-[1400px] mx-auto print:hidden">
                    ...
                    <div className="flex flex-wrap gap-2">
                        <div className="relative">
                            <button
                                ...
                                className="bg-gray-100 hover:bg-gray-200 text-gray-700 px-4 py-2.5 rounded-lg flex items-center transition-all font-medium"
                            >
                        ...
                        <button
                            ...
                            className="flex items-center space-x-2 bg-purple-600 hover:bg-purple-700 text-white px-4 py-2 rounded-lg font-medium transition-all shadow-sm"
                        >
                            <RefreshCw size={18} />
                            <span>Synchronize API</span>
                        </button>
                        {selectedIds.length > 0 && (
                            <button
                                onClick={handleBulkDelete}
                                className="flex items-center space-x-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-medium transition-all shadow-sm animate-in fade-in slide-in-from-right-2"
                            >
                                <Trash2 size={18} />
                ...
                <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6 flex flex-col md:flex-row gap-4 items-center">
                    <div className="relative flex-1">
                        ...
                        <input
                            type="text"
                            placeholder="Search by MAWB, ULD, or HAWB..."
                            value={searchTerm}
                ...
                <div className="bg-white rounded-xl shadow-md border border-gray-200 overflow-hidden flex flex-col">
                    {/* Top Scrollbar Sync */}
                    <div
                        className="overflow-x-auto h-4 border-b border-gray-100 bg-gray-50/50"
                        onScroll={(e) => {
                            const tableContainer = document.getElementById('cargo-table-container');
                            if (tableContainer) tableContainer.scrollLeft = (e.target as HTMLDivElement).scrollLeft;
                        }}
                    >
                        ...
                        <table className="w-full text-left border-collapse min-w-[1500px]">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    ...
                                    {visibleColumns.uldNo && (
                                        <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.uldNo }}>
                                            ULD No.
                                            <div onMouseDown={(e) => handleResize('uldNo', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                        </th>
                                    )}
                                    <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.mawb }}>
                                        MAWB
                                        <div onMouseDown={(e) => handleResize('mawb', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                    </th>
                                    <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.hawb }}>
                                        HAWB
                                        <div onMouseDown={(e) => handleResize('hawb', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                    </th>
                                    <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.totalPcs }}>
                                        Total Pcs
                                        <div onMouseDown={(e) => handleResize('totalPcs', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                    </th>
                                    {visibleColumns.totalWeight && (
                                        <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.totalWeight }}>
                                            Total Weight
                                            <div onMouseDown={(e) => handleResize('totalWeight', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                        </th>
                                    )}
                                    {visibleColumns.goodsDescription && (
                                        <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.goodsDescription }}>
                                            Description
                                            <div onMouseDown={(e) => handleResize('goodsDescription', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                        </th>
                                    )}
                                    {visibleColumns.actualPcs && (
                                        <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.actualPcs }}>
                                            Actual Pcs
                                            <div onMouseDown={(e) => handleResize('actualPcs', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                        </th>
                                    )}
                                    {visibleColumns.releasedPcs && (
                                        <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.releasedPcs }}>
                                            Released Pcs
                                            <div onMouseDown={(e) => handleResize('releasedPcs', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                        </th>
                                    )}
                                    {visibleColumns.rejectedPcs && (
                                        <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider relative group" style={{ width: columnWidths.rejectedPcs }}>
                                            Rejected Pcs
                                            <div onMouseDown={(e) => handleResize('rejectedPcs', e)} className="absolute right-0 top-0 h-full w-1 cursor-col-resize bg-transparent group-hover:bg-blue-300 transition-colors" />
                                        </th>
                                    )}
                                    ...
                                    <th className="px-4 py-4 text-xs font-bold text-gray-600 uppercase tracking-wider text-right">Actions</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr>
                                        <td colSpan={12} className="px-6 py-12 text-center text-gray-500">
                                            <div className="flex flex-col items-center">
                                                ...
                                                <p className="text-sm">Add new data or import from XLS to get started.</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                        ...
                                        <tr key={cargo.id} className={`hover:bg-blue-50/30 transition-colors ${selectedIds.includes(cargo.id) ? 'bg-blue-50/50' : ''}`}>
                                            <td className="px-4 py-3">
                                                <input
                                                    ...
                                                    onChange={() => handleSelectRow(cargo.id)}
                                                />
                                            </td>
                                            ...
                                            <td className="px-4 py-3 text-sm font-bold text-blue-600 truncate" style={{ maxWidth: columnWidths.mawb }}>{cargo.mawb}</td>
                                            ...
                                            {visibleColumns.actualPcs && (
                                                <td className="px-4 py-3 text-sm font-bold text-blue-600 font-mono italic truncate" style={{ maxWidth: columnWidths.actualPcs }}>
                                                    {getCalculatedPcs(cargo.mawb, cargo.hawb).actual}
                                                </td>
                                            )}
                                            {visibleColumns.releasedPcs && (
                                                <td className="px-4 py-3 text-sm font-bold text-green-600 font-mono truncate" style={{ maxWidth: columnWidths.releasedPcs }}>
                                                    {getCalculatedPcs(cargo.mawb, cargo.hawb).released}
                                                </td>
                                            )}
                                            {visibleColumns.rejectedPcs && (
                                                <td className="px-4 py-3 text-sm font-bold text-red-600 font-mono truncate" style={{ maxWidth: columnWidths.rejectedPcs }}>
                                                    {getCalculatedPcs(cargo.mawb, cargo.hawb).rejected}
                                                </td>
                                            )}
                                            {visibleColumns.remarks && <td className="px-4 py-3 text-sm text-gray-600 italic truncate" style={{ maxWidth: columnWidths.remarks }}>{cargo.remarks || '-'}</td>}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                            <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between bg-gray-50">
                                ...
                                <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 p-1 rounded-full hover:bg-gray-200 transition-all">
                                    <X size={24} />
                                </button>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-6 overflow-y-auto flex-1">
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <div className="space-y-1">
                                        <label className="text-sm font-semibold text-gray-700">ULD No.</label>
                                        <input name="uldNo" value={formData.uldNo} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. ULD12345" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-sm font-semibold text-gray-700">MAWB (SMU)</label>
                                        <input required name="mawb" value={formData.mawb} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none font-bold" placeholder="e.g. 126-12345678" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-sm font-semibold text-gray-700">HAWB</label>
                                        <input name="hawb" value={formData.hawb} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. H12345" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-sm font-semibold text-gray-700">Total Pcs</label>
                                        <input type="number" required name="totalPcs" value={formData.totalPcs} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-sm font-semibold text-gray-700">Total Weight (kg)</label>
                                        <input type="number" step="0.01" required name="totalWeight" value={formData.totalWeight} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                                    </div>
                                    <div className="space-y-1">
                                        <label className="text-sm font-semibold text-gray-700">Actual Pcs</label>
                                        <input type="number" name="actualPcs" value={formData.actualPcs} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" />
                                    </div>
                                    <div className="space-y-1 lg:col-span-2">
                                        <label className="text-sm font-semibold text-gray-700">Goods Description</label>
                                        <input required name="goodsDescription" value={formData.goodsDescription} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="e.g. Spare Parts" />
                                    </div>
                                    <div className="space-y-1">
                                ...
                                <div className="mt-8 flex justify-end space-x-3">
                                    <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-100 transition-all font-medium">
                                        Cancel
                                    </button>
                                    <button type="submit" className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-all font-bold shadow-md">
                                        {editingCargo ? 'Update Cargo' : 'Save Cargo'}
                                    </button>
                                </div>
                            </form>
                        </div>
            ...
            {printingCargo && (
                <div className="hidden print:flex fixed inset-0 bg-white z-[9999] text-black font-sans items-center justify-center">
                    <div className="w-[10cm] h-[10cm] border-4 border-black p-4 flex flex-col relative bg-white">
                        <div className="text-2xl font-black text-center border-b-4 border-black pb-2 mb-2 tracking-widest">
                            IATA CARGO LABEL
                        </div>
                        <div className="flex justify-between font-bold text-lg mb-4">
                            <span>AIRLINE: {printingCargo.airline || 'GA'}</span>
                            <span>{printingCargo.origin || 'CGK'}</span>
                        </div>
                        ...
                        <div className="flex justify-between items-center text-center mb-6 border-y-2 border-black py-2">
                            <div className="flex-1 border-r-2 border-black">
                                ...
                                <div className="text-3xl font-black">{printingCargo.destination || 'SUB'}</div>
                            </div>
                            <div className="flex-1 border-r-2 border-black">
                                <div className="text-xs font-bold">TOTAL PCS</div>
                                <div className="text-3xl font-black">{printingCargo.totalPcs}</div>
                            </div>
                            <div className="flex-1">
                                <div className="text-xs font-bold">WEIGHT</div>
                                <div className="text-3xl font-black">{printingCargo.totalWeight}K</div>
                            </div>
                        </div>
                        {printingCargo.hawb && (
                            <div className="mt-2 text-center flex flex-col items-center">
                                <div className="text-lg font-bold mb-1">
                                    HAWB: <span className="text-2xl">{printingCargo.hawb}</span>
                                </div>
                                <Barcode value={printingCargo.hawb} format="CODE128" width={1.5} height={30} displayValue={false} margin={0} />
                            </div>
                        )}
                        <div className="mt-auto text-[10px] text-center font-bold">
